<x-app-layout>
  ...
  <div class="container px-5 py-24 mx-auto">
    ...
    <form form method="post" action="{{ route('owner.films.update',['film' => $filmData->id])}}" enctype="multipart/form-data">
            @csrf
            @method('put')
                ...
                <div class="flex flex-wrap -m-2">
                    ...
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" required value="{{ old('name',$filmData->name) }}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                    </div>
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            <label for="movie_time" class="leading-7 text-sm text-gray-600">上映時間<span class="text-xs text-red-600">※必須</br></label>
                            <input type="text" id="movie_time" name="movie_time" required  value="{{ old('movie_time',$filmData->movie_time) }}" class="w-1/4 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">分
                        </div>
                    </div>
                        ...
                        <div class="relative">
                            <div class=" leading-7 text-sm text-gray-600">カテゴリー<span class="text-xs text-red-600">※必須</div>
                                <div class="w-full flex justify-around">
                                    <input type="radio" name="category" value=1  {{ old('category', $filmData->category) === 1 ? 'checked' : '' }}>アクション映画
                                    <input type="radio" name="category" value=2 {{ old('category', $filmData->category) === 2 ? 'checked' : '' }}>恋愛
                                    <input type="radio" name="category" value=3 {{ old('category', $filmData->category) === 3 ? 'checked' : '' }}>アドベンチャー映画
                                    <input type="radio" name="category" value=4 {{ old('category', $filmData->category) === 4 ? 'checked' : '' }}>ファンタジー映画
                                    <input type="radio" name="category" value=5 {{ old('category', $filmData->category) === 5 ? 'checked' : '' }}>ホラー映画
                                    <input type="radio" name="category" value=6 {{ old('category', $filmData->category) === 6 ? 'checked' : '' }}>サスペンス/クライムサスペンス映画
                                    <input type="radio" name="category" value=7 {{ old('category', $filmData->category) === 7 ? 'checked' : '' }}>ミステリー映画
                                    <input type="radio" name="category" value=8 {{ old('category', $filmData->category) === 8 ? 'checked' : '' }}>戦争映画
                                    <input type="radio" name="category" value=9 {{ old('category', $filmData->category) === 9 ? 'checked' : '' }}>SF映画
                                    <input type="radio" name="category" value=10 {{ old('category', $filmData->category) === 10 ? 'checked' : '' }}>ドキュメンタリー
                                </div>
                        </div>
                    </div>
